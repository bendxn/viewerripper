		...
		while (Process32Next(system_snapshot, &process) == TRUE) {
			...
			if (_stricmp(process.szExeFile, dmm_process_name) == 0) {
				...
				for (page = 0; !done; ++page) {
					...
					FIBITMAP* bitmap = FreeImage_Allocate(w, h, 24, 0, 0, 0);
					
					output_name = malloc(sizeof(char) * (strlen(output_prefix) + number_length + strlen(".jpeg") + 1));

					sprintf(output_name, "%s%0*i.jpeg", output_prefix, padwidth, page + offset);
				...
				/*	if (!output_file) {
					...
					for (i = 0; i < h; ++i) {
						int j;
						for (j = 0; j < w; ++j) {
							triplet = (BYTE*)(image + i * w + j);
							color.rgbtBlue = triplet[0];
							color.rgbtGreen = triplet[1];
							color.rgbtRed = triplet[2];
							FreeImage_SetPixelColor(bitmap, j, i, &color);
						}
					}
					FreeImage_FlipVertical(bitmap);
					FreeImage_Save(FIF_JPEG, bitmap, output_name, JPEG_QUALITYSUPERB);
